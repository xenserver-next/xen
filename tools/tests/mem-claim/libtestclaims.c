...
int lib_claim_memory(struct test_ctx *ctx, uint32_t domid, uint32_t records,
                     memory_claim_t *claim_set, const char *reason)
{
    lib_set_step(ctx, "%s", reason);
...
int lib_expect_claim_memory_failure(struct test_ctx *ctx, uint32_t domid,
                                    uint32_t records, memory_claim_t *claim_set,
                          ...
                          unsigned int spare)
{
    ...
    return lib_claim_memory(
        ...
        &(memory_claim_t){
            .pages  = claim_pages,
            .target = XEN_DOMCTL_CLAIM_MEMORY_UNPINNED
        },
        ctx->result->params);
}
                          ...
                          unsigned int node, unsigned int spare)
{
    ...
    rc = lib_claim_memory(
        ctx, domid, 1,
        &(memory_claim_t){ .pages = claim_pages, .target = node },
        ctx->result->params);
    if ( rc )
        return rc;
