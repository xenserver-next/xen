                                      ...
                                      unsigned int line)
{
    ...
    if ( node >= 0 )
    {
        rc = get_node_free_pages(node, &node_free);
        if ( rc )
            printf("L%u:     Memory: free=%"PRIu64", claimed=%"PRIu64""
                   " (node %d free unavailable: %d - %s)\n",
...
static int wrap_claim_pages(xc_interface *xch,
                            ...
                            unsigned long pages)
{
    log_check("Check xc_domain_claim_pages() accepts global claim request");
                                  ...
                                  unsigned long pages)
{
     ...
     */
    log_check("Check xc_domain_claim_memory() rejects nr_claims=2 for now");
    ...
    log_check("Check xc_domain_claim_memory() rejects excessive nr_claims");
    rc = xc_domain_claim_memory(xch, domid, XEN_DOMCTL_MAX_CLAIMS + 1, claims);
    if ( rc != -1 || errno != EINVAL )
    {
