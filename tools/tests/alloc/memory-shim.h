 ...
 */
#ifndef _TEST_ALLOC_MEMORY_SHIM_
#define _TEST_ALLOC_MEMORY_SHIM_
...
/* memory.c uses list.h */
#include "../rangeset/list.h"
 ...
 */
#define gfn_to_mfn(d, gfn)                                    ((mfn_t)(~0UL))
 ...
 * __XEN_FRAME_NUM_H__; gfn_t is a plain unsigned long in the test harness.
 */
#define INVALID_GFN_RAW (~0UL)
#define INVALID_GFN            ((gfn_t)(~0UL))
#define gfn_eq(x, y)           ((unsigned long)(x) == (unsigned long)(y))
...
/*
 * p2m_type_t and related constants: asm/p2m.h is blocked by
 * _XEN_ASM_X86_P2M_H.  Provide a minimal subset used by the non-x86
 * compile paths in memory.c that are not guarded by CONFIG_X86.
 */
...
{
    (void)d; (void)space; (void)extra; (void)idx; (void)gfn;
    return -EOPNOTSUPP;
}
