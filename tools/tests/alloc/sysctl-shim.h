 ...
 */
#ifndef _TEST_ALLOC_SYSCTL_SHIM_
#define _TEST_ALLOC_SYSCTL_SHIM_
...
/* domain_page.h re-includes xen/mm.h which is blocked by __XEN_MM_H__. */
...
/*
 * per_cpu(var, cpu): sysctl.c passes the result to cpumask_weight()
 * which is already stubbed to return 1 regardless of its argument.
 * Return a static cpumask_t so the expression is well-typed.
 */
static cpumask_t __used sysctl_test_cpumask = ~0UL;
#define per_cpu(var, cpu)               sysctl_test_cpumask

/*
 * Forward declarations for page_alloc.c functions.
 *
 * These are declared in xen/mm.h (blocked by __XEN_MM_H__) but defined
 * in the page_alloc.c translation unit already compiled into this unit.
 */
int online_page(mfn_t mfn, uint32_t *status);
int query_page_offline(mfn_t mfn, uint32_t *status);
void get_outstanding_claims(uint64_t *free_pages,
...
/*
 * opt_gnttab_max_version: xen/grant_table.h is blocked by
 * __XEN_GRANT_TABLE_H__ in memory-shim.h.  No grant tables in tests.
 */
