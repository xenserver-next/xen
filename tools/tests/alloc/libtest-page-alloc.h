...
static void init_numa_node_data(unsigned int start_mfn)
{
    ...
    for ( unsigned int i = 0; i < MAX_NUMNODES; i++ )
    {
        node_data[i].node_start_pfn = start_mfn + (i * 16);
        node_data[i].node_present_pages = 16UL;
        node_data[i].node_spanned_pages = 16UL;
    }
    memnode_shift = 4;
...
#define test_page_list_add_node_buddy(node, start_mfn, order)             \
        page_list_add_buddy(test_pages + node_data[node].node_start_pfn + \
