...
/*
 * Test basic host-wide (not node-affine) functionality of memory claims,
 ...
 */
#define TEST_WRAP_XEN_COMMON_DOMCTL_C /* This test also tests domctl.c code */
#include "libtest-page-alloc.h"

/* Install a host-wide claim entry using the direct page_alloc function call */
int install_host_claims_direct(struct domain *d, unsigned long pages)
{
    memory_claim_t claim_set[] = {
        { .target = XEN_DOMCTL_CLAIM_MEMORY_HOST, .pages = pages },
    };

    return domain_set_claim_entries(d, ARRAY_SIZE(claim_set), claim_set);
}

/* Install a host-wide claim entry using the domctl helper for the hypercall */
int install_host_claims_domctl(struct domain *d, unsigned long pages)
{
    memory_claim_t claim_set[] = {
        { .target = XEN_DOMCTL_CLAIM_MEMORY_HOST, .pages = pages },
    };
...
static void test_cancel_claims(int start_mfn)
{
    ...
    ASSERT(install_host_claims(dom2, FREE_PAGES) == 0);
