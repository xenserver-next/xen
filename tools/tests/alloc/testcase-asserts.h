...
#ifndef _RELDIR_
#define _RELDIR_ "tools/tests/alloc"
#endif
...
static void testcase_print_summary(void)
{
    ...
    if ( *testcase_assert_enabled_tests )
    {
        ...
        for ( char **p = testcase_assert_enabled_tests; *p; p++ )
        {
            ...
            if ( !found )
                fprintf(stderr, " (WARNING: test not found)");
        }
        fprintf(stderr, "\n");
    }
}
...
static int test_complete(void)
{
    ...
    printf("\nTest program %s for %s completed.\n", testcase_program_name,
           uts.machine);
    return 0;
}
...
static const char *parse_args(int argc, char *argv[], const char *topic)
{
    ...
    /* Print some info about the test program */
    uname(&uts);
    printf("Program: %s\n", program_name);
    printf("Topic  : %s\n", topic);
