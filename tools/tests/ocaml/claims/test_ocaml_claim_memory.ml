...
let make_domain_config physinfo =
...

  {
    ...
    (* Grant table protocol version; 1 = legacy format; universally supported *)
    max_grant_version = 1;
  ...
  if Int64.compare free_pages_pre (Int64.add request_pages 1L) <= 0 then
    ...
    (try
      with_domain xch config (fun domid ->
          ...
          logf __LINE__ "domain_claim_memory domid=%d pages=%Ld node=any" domid claim;
          Xenctrl.domain_claim_memory xch domid
          ...
          with_domain xch config (fun other_domid ->
              ...
              expect_xc_error "host-wide claim blocks second-domain allocation"
                (fun () ->
                  logf __LINE__ "domain_memory_increase_reservation dom=%d" other_domid;
...
let run_node_claim_test xch physinfo config =
  ...
  try
    with_domain xch config (fun domid ->
       ...
       * Claiming only 1 page here would cause domain_memory_increase_reservation
       * to fail because the claim would be exhausted after the first page.
       *)
      let node = 0l in
      logf __LINE__ "domain_claim_memory domid=%d pages=%Ld node=%ld" domid request_pages node;
      Xenctrl.domain_claim_memory xch domid
