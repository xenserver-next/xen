...
/* Get and set memory claims for a domain */
int xc_domain_claim_memory(xc_interface *xch, uint32_t domid, uint32_t mode,
                           uint32_t *nr_entries, memory_claim_t *claim_set)
{
    struct xen_domctl domctl = {};
    DECLARE_HYPERCALL_BOUNCE(claim_set, sizeof(*claim_set) * *nr_entries,
                             XC_HYPERCALL_BUFFER_BOUNCE_BOTH);
    int ret;
...
/* This hypercall is deprecated and replaced by xc_domain_claim_memory() */
