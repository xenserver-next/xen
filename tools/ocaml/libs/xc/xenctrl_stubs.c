...
/* Marshalling of memory claim sets for calling xc_domain_claim_memory() */
                                           ...
                                           value claim_set)
{
	...
	/* claim_set is a claim array, where each entry is a claim record {pages, node} */
	for (mlsize_t i = 0; i < nr_entries; i++) {
		...
		c_claim_set[i] = (memory_claim_t) {
			.pages = (uint64_t)pages,
			.target = (node == -1)
				? XEN_DOMCTL_CLAIM_MEMORY_UNPINNED
				: (uint32_t)node
		};
	}
