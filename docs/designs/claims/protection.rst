...
This means that the request is not for a domain, or the :c:expr:`memflags`
of the request includes :c:expr:`MEMF_no_refcount`.
...
Example callers which use :c:expr:`MEMF_no_refcount` when allocating memory
or use :c:expr:`MEMF_no_owner` which also sets :c:expr:`MEMF_no_refcount`
...
which use :c:expr:`MEMF_no_refcount` or :c:expr:`MEMF_no_owner` to
...
Before starting, the allocator takes the global :c:expr:`heap_lock`.
...
Exact-node request check
========================

The first check the allocator performs is a check protecting floating claims
which are part of the total pool of the claims on the entire host.

If the request is reference-counted, the request is an exact-node request
and the claims of the domain for that node cover the request, the request
is only eligible for this exact node, and can skip the check for the
protection of floating claims in the next section.

Protection of floating claims
============================

1. Get the total amount of unclaimed memory available in the system.
   It is the sum of the free pages on all NUMA nodes (``total_avail_pages``)
   minus the total amount of claimed memory across all domains
   (``outstanding_claims``) this includes all floating claims
   and all node-specific claims.

2. Check whether the request can be satisfied by the unclaimed memory itself.

   If so, the allocation calls :c:expr:`get_free_buddy()` to perform the
   node-specific checks and find free pages on the appropriate node(s)
   to satisfy the request.

   This is the common case, especially for smaller allocations and in general,
   when the system is not under heavy memory pressure and not fully-claimed
   at the moment.
...
2. If the total claims of the domain (``d->floating_claims + d->node_claims``)
...
After passing the floating claims protection check, the allocator calls
:c:expr:`get_free_buddy()` to pick nodes for allocation and check the
node's suitability for this request:

1. If the request is reference-counted and the request is covered by
   the claims of the domain for that node, the allocation can proceed.

2. If the request does not proceed based on the above check, the amount
   of unclaimed memory available on that node is calculated, which is the
...
3. If the request can be satisfied by the sum of the unclaimed memory
...
4. If the allocation is an exact-node request, or the allocator
   has no further nodes to consider, the allocation fails.

5. Else, if the allocator has to consider further nodes for this request,
