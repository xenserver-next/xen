...
int claim_memory(struct domain *d, const struct xen_domctl_claim_memory *uinfo)
{
    ...
    /* Prevent claim truncation when passed to domain_set_outstanding_pages() */
    if ( claim.pages > (uint64_t)(typeof(d->outstanding_pages))-1 ||
         claim.node > (uint32_t)(nodeid_t)-1 || claim.pad )
        return -ERANGE;
