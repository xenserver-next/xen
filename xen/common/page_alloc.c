                             ...
                             memory_claim_t *claim_set)
{
    ...
    for ( int i = 0; i < nr_claims; i++ )
    {
        ...
        if ( nodemask_test(node, &nodes) ||   /* Duplicate node in claim set */
             pages > ((typeof(d->node_claims))-1) - node_claims ) /* overflow */
            goto out;
        node_set(node, nodes);

        if ( pages > (uint64_t)(node_avail_pages[node] -
                                node_outstanding_claims[node]) +
                               d->claims[node] ) /* current claim deduction */
        {
    ...
    for ( int i = 0; i < nr_claims; i++ )
    {
        ...
        d->claims[node] = pages;
        node_outstanding_claims[node] += pages;
    }
    ret = 0;
 out:
