                                 ...
                                 memory_claim_t *claims)
{
    ...
    else
    {
        ...
        {
            node = claims[i].node;
            outstanding_claims += claims[i].pages;
            node_outstanding_claims[node] += claims[i].pages;
            d->claims[node] = claims[i].pages;
        }
    }
    ret = 0;
...
/* Answer if memory and claims permit this request to proceed */
static inline bool protect_outstanding_claims(const struct domain *d,
                                        ...
                                        const struct domain *d)
{
    ...
    for ( ; ; )
    {
        ...
        if ( !protect_outstanding_claims(d, node_avail_pages[node],
                                         node_outstanding_claims[node],
    ...
    unsigned long allocation, nodeid_t alloc_node)
{
    ...
    if ( !d )
        return;
    if ( d->outstanding_pages ) /* d has host-claimed pages, release them */
    {
    ...
    else
    {
        ...
        if ( consume )
        {
            ASSERT(total_avail_pages >= consume);
            ASSERT(node_avail_pages[alloc_node] >= consume);
            ASSERT(outstanding_claims >= consume);
            outstanding_claims -= consume;
            node_outstanding_claims[alloc_node] -= consume;
            d->claims[alloc_node] -= consume;
        }
        consume = allocation - consume;
        if ( !consume )
            return;

        /* Compute sum of remaining per-node claims. */
        remaining_claims = 0;
        for ( node = 0; node < MAX_NUMNODES; node++ )
        ...
        /*
         * The domain has claims, but not on alloc_node. If the allocation
         * would exceed the domain's max_pages, reduce claims on other nodes
         * up to the excess over max_pages so we don't reduce claims more
         * than necessary to honor the max_pages limit.
         */
    ...
    struct domain *d)
{
     ...
     */
    if ( !protect_outstanding_claims(d, total_avail_pages, outstanding_claims,
