 ...
 * This is used to ensure that the domain has a guaranteed amount of
 * memory available to it, even if the system is under memory pressure.
 * At the moment, only the legacy claim type XEN_DOMCTL_CLAIM_MEMORY_LEGACY
 * is supported, which does not specify a NUMA node and is satisfied from
 * any available memory on the system.
 */
...
#define XEN_DOMCTL_CLAIM_MEMORY_MAX_CLAIMS  1U /* One global host-wide claim */
